 ...
 * así que la validación de rol/pertenencia vive acá, igual que en toda
 * mutación sensible — ver lib/auth/requireRole.ts):
 * - TRABAJADOR: solo su propia inspección (mismo criterio que
 *   `getOwnInspectionOrNotFound`, lib/inspections/queries.ts).
 * - SUPERVISOR, DIRECTOR, SST: cualquier inspección (mismo criterio que
 *   `getInspeccionDetalleOrNotFound`/`getInspeccionDetalleForOversight`,
 *   lib/inspections/supervisor-queries.ts — no hay asignación
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const esDueño = role === "TRABAJADOR" && inspection.workerId === session.user.id;
  const esRevisor = role === "SUPERVISOR" || role === "DIRECTOR" || role === "SST";
  if (!esDueño && !esRevisor) {
    return NextResponse.json({ error: "Inspección no encontrada." }, { status: 404 });
  }
